...
From 0.16.1 the shared live ARM/Graph transport reads at most 8 MiB plus one sentinel byte per response. Vault metadata keeps its stricter 4 MiB limit. Over-limit bodies are closed immediately, are not retried, and produce the safe `response_size_limit` code; incomplete inventory and per-check error states remain visible in archives and PDFs. UTF-8 JSON with duplicate object members, nonfinite numbers or nesting beyond 64 levels is rejected as `malformed_response`. No response/error body is retained by these transport failures.

These are per-response limits, not a total-memory, total-population or wall-clock guarantee. Paged collectors retain their configured page limits and adapter-specific population limits; use scoped runs and the measured runtime budget. This change does not add resumable collection. New safe error codes require a reader at least as new as the generating release; older archives remain readable without rewriting them.
