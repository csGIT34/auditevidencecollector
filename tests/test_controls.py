...
def sample(check, negative=False):
    if check.kind in ('vmss_instances','container_revisions','backup_jobs'):return []
    ...
    for number, rt in enumerate(sorted({c.resource_type for c in CHECKS.values() if not c.resource_type.startswith('graph.')})):
        ...
        for c in checks:
            ...
            if c.operation == 'diagnostic_routes':
                ...
                criteria[c.id]={'operator':'equals','value':value}
                continue
            if c.operation=='backup_jobs':
                from tests.test_backup_jobs import job,normalized
                item=job(row['id'],c)
                responses[endpoint(row['id']+c.suffix,c.api)]={'value':[item]}
                criteria[c.id]={'operator':'equals','value':[normalized(item,c)]}
                continue
            if c.operation=='container_revisions':
                from tests.test_container_revisions import revision,normalized
                item=revision(row['id'])
                responses[endpoint(row['id']+c.suffix,c.api)]={'value':[item]}
                criteria[c.id]={'operator':'equals','value':[normalized(item)]}
                continue
            if c.operation=='vmss_instances':
                row['properties']['orchestrationMode']='Uniform'
                instance={'id':row['id']+'/virtualMachines/0','properties':{'latestModelApplied':True}}
                responses[endpoint(row['id']+c.suffix,c.api)]={'value':[instance]}
                criteria[c.id]={'operator':'equals','value':[{'instance_id':instance['id'].lower(),'latest_model_applied':True}]}
                continue
    ...
    def test_every_predicate_missing_null_invalid_is_unknown_and_redacted(self):
        for value in (None, {'secret':'DO-NOT-ARCHIVE-SECRET'}, 'DO-NOT-ARCHIVE-SECRET'):
            ...
            for url in list(responses):
                if '/diagnosticSettings?' in url or '/federatedIdentityCredentials?' in url or '/roleAssignments?' in url or '/backupInstances?' in url or '/backupProtectedItems?' in url or '/virtualMachines?' in url or '/revisions?' in url or '/backupJobs?' in url:responses[url]={'value':[{'properties':{'logs':value}}]}
