...
One new `rg-<lab_name>` in East US; one Python 3.12 Linux FC1 Function App; one explicitly attached UAMI; separate Standard LRS Hot runtime/evidence accounts, with deployment `package` and retained `evidence` containers. Host-created key/lease containers stay in runtime storage. There are no paid test databases, VMs, logging destinations or networking appliances. Sixteen Terraform resource instances are expected in the initial plan. The validated HTTP concurrency is 16 requests per instance; the maximum stays one 2-GiB on-demand instance per function group, with no always-ready instances. Low HTTP concurrency reproduced sequential-request 503s in the personal lab; see [the follow-up validation](REPEAT_VALIDATION.md).
...
3. Choose one explicit UTC calendar time **outside the short manual test session** (e.g. a day/month/hour within the seven-day window, never `* * * * * *`). Set that six-field `CG_COLLECTION_SCHEDULE` and `CG_COLLECTION_ENABLED=true`, leaving the expiry and `CG_RESOURCE_GROUP` unchanged. Verify `CollectEvidence` indexes. An ARM site state of Running is not host readiness: after an app-settings restart, first require the key-protected host status to report Running and the runtime function list to include `CollectEvidence`. Keep the master key in hidden input/memory when inspecting these administrative endpoints. Then use the supported `POST .../sites/LAB_APP/syncfunctiontriggers?api-version=2024-04-01` action for this exact lab app if ARM metadata is stale, and reread its function list. An early synchronization returned a host error during testing; warming/checking the host before synchronization resolved registration. Retry only bounded metadata/readiness checks, never an uncertain audit invocation; do not invoke an unindexed target. Manually trigger it once using `lab_request.py --operation collect` and the lab master key; this administrative key must stay in hidden input and is not used for normal reports. [Manual timer invocation](https://learn.microsoft.com/en-us/azure/azure-functions/functions-manually-run-non-http). Disable collection and clear the schedule immediately after completion; don't leave the future calendar trigger active. If any acknowledgement is lost, inspect manifests/invocation provenance before retrying.
