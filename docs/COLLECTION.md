...
The synthetic example and automated tests do not execute Azure commands. Offline command tests use a temporary fake `az` program to check shell argument boundaries. An optional local JMESPath test can run with an installed Azure CLI Python runtime to validate projection syntax/results without importing the Azure CLI, obtaining tokens or accessing Azure. Standard-library-only environments skip that optional parser check; PDF and Azure hosting tests also require their optional extras; use requirements-dev.txt and scripts/validate.py for the full no-skips gate.

## Optional resource-group scope (0.3.1)

Use `collect --subscription UUID --resource-group NAME` or hosted `CG_RESOURCE_GROUP` with exactly one explicit subscription. Only that RG's inventory and supported children are read; out-of-group IDs and changed pagination are rejected. The group is saved in snapshot inventory and historical reports. The accepted portable name uses ASCII letters, digits, underscore, parentheses, hyphen and period, up to 90 characters, with no trailing period. This is intentionally narrower than every possible Azure Unicode name. Subscription-wide behavior is unchanged when the option is absent. Metadata preflight still needs `Microsoft.Resources/subscriptions/read`; RG Reader does not replace that narrow tenant-check permission.

Scoped snapshots add optional `inventory.subscriptions[].resource_group` to schema 1.0. Version 0.3.1 reads both old and scoped snapshots; older strict validators may reject the new optional field, so use 0.3.1 or newer to replay scoped runs. Old archived facts are not rewritten.
