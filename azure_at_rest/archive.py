...
CRITERIA = {
    ...
    'managed_redis': 'Successful identity read must return a reviewed Azure Managed Redis / Redis Enterprise SKU family; any other or missing SKU is unknown. Databases must be completely enumerated and individually assessed. The guarantee covers cluster persistence, export-temporary and OS disks, plus the Flash families\' transient NVMe disk; in-memory data and exported copies are excluded.',
    'redis_database': 'The owning cluster must have a successful detail read and a reviewed SKU family before its disk guarantee is applied to this database\'s declared RDB/AOF files. In-memory keys and values, geo-replication targets and exported copies are excluded.',
    'redis': 'A returned tier, family and size select the documented statement: Basic and Standard C0/C1 have no disk encryption and fail; an enabled rdb/aof flag, or missing Premium persistence settings, leaves the destination unresolved and unknown because it is configured through a connection string this rule never reads; otherwise the mounted OS disk guarantee applies to a cache with no persistence destination.',
