...
Use a new directory outside the checkout. The command collects synthetic ARM and Graph responses through the real adapters, evaluates explicit fictional criteria, archives the run and renders a self-contained PDF plus JSON/Markdown. It makes no Azure or Graph calls. The supplied case exercises every predicate: 198 expected PASS and one expected expired-credential FAIL. These are software test expectations, not a recommended security baseline or workplace approval.

The example inputs are reproducible HTTP fixtures and fictional criteria. They deliberately include secret canaries to verify projection; no real tenant IDs or credentials are present.
...
Coverage is scoped: Event Grid currently means custom topics, Managed Redis covers the cluster and its database children but not access policies or private connectivity, VMSS fields are model-level, and diagnostic settings are configuration rather than event-delivery proof. Container/guest operating evidence, effective privileges, additional service variants/children, backup populations and restore records, manual/inherited evidence workflows, native Wiz integration and workplace acceptance remain open. A candidate objective can require several of these facts together. Refer to the full completion plan, service backlog and delivery register rather than interpreting predicate count as objective completion.
...
| REDIS-tls | REDIS-T | Minimum configured TLS version; database protocol remains separate: `minimumTlsVersion` | [Definition](https://learn.microsoft.com/en-us/azure/templates/microsoft.cache/2025-04-01/redisenterprise) |
| REDIS-high-availability | REDIS-B | Cluster replication configuration; recovery testing remains separate: `highAvailability` | [Definition](https://learn.microsoft.com/en-us/azure/templates/microsoft.cache/2025-04-01/redisenterprise) |
| REDIS-access-keys | REDIS-I | Database access-key authentication configuration; Entra role assignments remain separate: `accessKeysAuthentication` | [Definition](https://learn.microsoft.com/en-us/azure/templates/microsoft.cache/2025-04-01/redisenterprise/databases) |
| REDIS-client-protocol | REDIS-T | Configured client connection protocol: `clientProtocol` | [Definition](https://learn.microsoft.com/en-us/azure/templates/microsoft.cache/2025-04-01/redisenterprise/databases) |
| REDIS-clustering-policy | REDIS-C | Database clustering policy; client compatibility remains separate: `clusteringPolicy` | [Definition](https://learn.microsoft.com/en-us/azure/templates/microsoft.cache/2025-04-01/redisenterprise/databases) |
| REDIS-eviction-policy | REDIS-C | Database eviction policy; data-loss impact remains an owner judgement: `evictionPolicy` | [Definition](https://learn.microsoft.com/en-us/azure/templates/microsoft.cache/2025-04-01/redisenterprise/databases) |
| REDIS-defer-upgrade | REDIS-V | Deferred Redis version upgrade configuration; the installed version remains separate: `deferUpgrade` | [Definition](https://learn.microsoft.com/en-us/azure/templates/microsoft.cache/2025-04-01/redisenterprise/databases) |
| REDIS-rdb-persistence | REDIS-B | RDB persistence configuration; snapshot recovery testing remains separate: `persistence.rdbEnabled` | [Definition](https://learn.microsoft.com/en-us/azure/templates/microsoft.cache/2025-04-01/redisenterprise/databases) |
| REDIS-aof-persistence | REDIS-B | AOF persistence configuration; recovery testing remains separate: `persistence.aofEnabled` | [Definition](https://learn.microsoft.com/en-us/azure/templates/microsoft.cache/2025-04-01/redisenterprise/databases) |
...
| REDIS-diagnostic-logs-database | REDIS-L | Enabled diagnostic log categories/groups; configured destinations and ingestion health require separate evidence: `logs[].category/categoryGroup` | [Definition](https://learn.microsoft.com/en-us/rest/api/monitor/diagnostic-settings/list?view=rest-monitor-2021-05-01-preview) |
...
| APPREG-implicit-access-token | APPREG-T | Implicit access token issuance setting; the deprecated implicit flow returns tokens through the browser: `web.implicitGrantSettings.enableAccessTokenIssuance` | [Definition](https://learn.microsoft.com/en-us/graph/api/resources/implicitgrantsettings?view=graph-rest-1.0) |
| APPREG-implicit-id-token | APPREG-T | Implicit ID token issuance setting: `web.implicitGrantSettings.enableIdTokenIssuance` | [Definition](https://learn.microsoft.com/en-us/graph/api/resources/implicitgrantsettings?view=graph-rest-1.0) |
| APPREG-redirect-transport | APPREG-N | Transport of every declared redirect URI; the URIs themselves are not retained: `web.redirectUris transport` | [Definition](https://learn.microsoft.com/en-us/graph/api/resources/webapplication?view=graph-rest-1.0) |
...
| REDIS-approved-arm-grants-database | REDIS-I | Declared ARM assignments at or above this resource and resolved role permissions; effective access remains separate: `assignments[].principalId/scope/roleDefinitionId/permissions/conditionSha256` | [Definition](https://learn.microsoft.com/en-us/rest/api/authorization/role-assignments/list-for-scope?view=rest-authorization-2022-04-01) |
...
## Additional scoped encryption rules (0.18.0)

Azure Managed Redis and the retiring Azure Cache for Redis are assessed as separate exact types; neither inherits the other's statement. The cluster rule applies only to a returned reviewed SKU family, and the database rule applies only once its owning cluster is verified.

| Exact ARM type | Pinned detail API | Covered boundary and source | Explicit remaining boundary |
| --- | --- | --- | --- |
| Microsoft.Cache/redisEnterprise | 2025-04-01 | [Cluster persistence, export-temporary and OS disks under Microsoft-managed keys, plus the Flash families' transient NVMe disk](https://learn.microsoft.com/en-us/azure/redis/how-to-encryption) | In-memory keys and values; exported copies follow their destination store; an unlisted SKU family stays unknown |
| Microsoft.Cache/redisEnterprise/databases | 2025-04-01 | [Declared RDB/AOF files written to the owning cluster's encrypted disks](https://learn.microsoft.com/en-us/azure/redis/how-to-persistence) | In-memory keys and values, geo-replication link targets and exports; an unverified cluster leaves the database unknown |
| Microsoft.Cache/Redis | 2024-03-01 | [Mounted OS disk for a returned Basic, Standard or Premium tier and size](https://learn.microsoft.com/en-us/azure/azure-cache-for-redis/cache-how-to-encryption) | Basic and Standard C0/C1 fail because Microsoft documents no disk encryption there; an enabled rdb/aof flag leaves the persistence destination unresolved |

Data persistence destinations for Azure Cache for Redis are configured through storage connection strings. This collector never reads them, so an enabled persistence flag remains incomplete pending separate destination evidence rather than resolving a storage account. Databases are discovered through the cluster's child listing; a denied or truncated listing keeps the cluster incomplete. These rules are offline-tested and have not been live-verified.

## Additional scoped encryption rules (0.17.0)

These use the existing encryption result path, separate from the 188 configurable predicates. A successful matching resource detail read and the linked provider guarantee support PASS only for the stated stored-data boundary. Denied/mismatched reads and transitional provisioning cannot pass. No asset content, variable values, credentials or job streams are fetched.
