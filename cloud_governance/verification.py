...
GUIDANCE = {
    ...
    "safety": "Commands use management-plane GETs and selected metadata only; the one exception in collection itself is the Azure Policy compliance query, which Azure exposes only as a POST and which changes nothing. They do not provision resources, read application data, fetch secrets/keys, enable diagnostics or activate paid services. No command is executed while generating a report. Do not add --debug or remove the projection; normal ARM responses may contain incidental sensitive fields before CLI projection.",
