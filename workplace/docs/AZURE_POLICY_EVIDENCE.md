...
Azure Policy supplies provider-defined configuration evaluations and NIST SP 800-53 Rev. 5 mappings. This program reads all returned states as **provider-asserted evidence**. The intended architecture uses direct predicates for identified gaps; the current collector still runs direct collection before adding Policy.

Collection uses Policy Insights directly and does not depend on the Defender for Cloud dashboard. The assignment procedure below records an example configuration; assess the actual initiative version, effects and existing assignments before adapting it to work.
...
- **`Compliant` is not control satisfaction.** It means a definition's condition matched at evaluation time, in the same way a scoped predicate meeting its criterion is not control satisfaction. The control register keeps that distinction in its status vocabulary.
