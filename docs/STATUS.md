...
Version 0.22.0 passed 369 local tests with zero skips and completed a live lab collection: 7 resources, 531 policy records with no unreadable rows, 14 Compliant, 36 NonCompliant and 481 Unknown awaiting attestation. Indexing that run by control produced 330 controls with evidence and 34 carrying findings. The packaged control list is now the complete standard, verified against the pinned OSCAL digest, so no referenced control or enhancement is left unindexable. This is one lab subscription with seven resources; it demonstrates the pipeline, not workplace coverage.

Version 0.25.0 passed 388 local tests with zero skips. A hosted collection now carries Azure Policy compliance when an assignment is configured, archived as a third evidence kind in the same run with Microsoft's control mapping frozen into it. Hosted policy collection is offline-tested against fixtures; the same code path was live-verified through the CLI against the lab assignment. The setting is empty by default, so existing deployments are unchanged.

Version 0.26.0 passed 391 local tests with zero skips. Probing the live Policy Insights query showed it truncates to the requested `$top` and returns no continuation link, so a full page could not be distinguished from a truncated one; the lab returned exactly the number requested at every boundary tested. The query now requests one record beyond the limit it retains, and a truncated section is recorded as truncated and cannot conclude compliant. The lab run is well under the limit and reports `truncated: false`.
