...
The source is published at [csGIT34/auditevidencecollector](https://github.com/csGIT34/auditevidencecollector). [Linux/macOS CI](https://github.com/csGIT34/auditevidencecollector/actions/workflows/offline.yml) validates the offline gate and builds. The [personal follow-up validation](../infra/personal-lab/REPEAT_VALIDATION.md) resolved the observed HTTP 503s with a measured Flex concurrency configuration and completed a second live collection plus current/historical PDFs with preserved original hashes. These results establish personal-lab behavior, not workplace connectivity or acceptance.

## Runtime and package

Use an approved Linux x86_64 Python 3.12 Functions v4 environment; Flex Consumption uses `functionAppConfig.runtime`, deployment storage and scale/concurrency properties. macOS is development only. `host.json` chooses ten minutes; synchronous HTTP reports still need measured headroom under the platform HTTP limit. Keep storage, network paths, region, plan capacity, package feeds and cost/logging ownership within approved workplace policy. The personal Flex lab required `scaleAndConcurrency.triggers.http.perInstanceConcurrency=16` for reliable repeated requests while retaining `maximumInstanceCount=1`. Validate that combination and actual workload sizing in your work plan; it is a measured lab configuration, not a universal production sizing recommendation.
...
`CG_POLICY_ASSIGNMENT=<assignment name>` collects Azure Policy compliance for that assignment during the same run, using the host identity. It is empty by default. The name must be an existing assignment at the selected subscription scope, and exactly one subscription must be configured, because compliance is queried per subscription. The run reads the assignment and its initiative by GET to freeze Microsoft's control mapping into the evidence, then issues the single read-only query POST Azure requires for compliance state. A configured assignment whose compliance cannot be read fails the operation rather than archiving an empty result. See [Azure Policy as an evidence source](AZURE_POLICY_EVIDENCE.md).

Minimum permission for this setting is a reader role at the assessed scope; Policy Insights compliance reads require no additional grant beyond resource read access. No policy is created, changed or remediated.
